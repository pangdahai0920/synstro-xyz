...
const products = [
  { slug: 'network-cabinets', asset: productAssets.network, en: { title: 'Network Cabinets & Server Racks', eyebrow: 'Indoor infrastructure', body: 'Wall-mount cabinets, floor-standing cabinets, server cabinets, and data center racks based on the model list from the product parameter sheet.' }, zh: { title: '网络机柜与服务器机柜', eyebrow: '室内基础设施', body: '根据产品参数表整理壁挂机柜、落地机柜、服务器机柜和数据中心机柜系列，并按型号展示尺寸、体积、重量和配件。' } },
  { slug: 'pdu-power', asset: productAssets.pdu, en: { title: 'Rack PDU by Socket Standard', eyebrow: 'Power distribution', body: 'PDU directory organized by socket type, copper wire specification, rated current, outlet quantity, switch, and cable description.' }, zh: { title: '按插座标准分类的 PDU', eyebrow: '电源分配', body: '根据参数表按 Multi Universal、英标、美标、欧标、C13、C19 分类，展示线缆规格、额定电流和配置描述。' } },
  { slug: 'outdoor-telecom', asset: productAssets.outdoor, en: { title: 'Outdoor Waterproof Cabinets', eyebrow: 'Outdoor protection', body: 'Outdoor waterproof network cabinets and air-conditioning waterproof battery cabinets organized by model and cabinet size.' }, zh: { title: '户外防水柜与电池柜', eyebrow: '户外防护', body: '根据参数表整理户外防水网络机柜、空调防水电池柜，按型号展示柜体尺寸。' } }
];
...
const copy = {
  en: {
    ...
    switch: '中文', cta: 'Send inquiry', view: 'View models', catalog: 'Product catalog', series: 'Series', modelCount: 'models', productModels: 'Product models', contactTitle: 'Tell us what your project needs.',
  ...
  zh: {
    ...
    switch: 'English', cta: '发送询盘', view: '查看型号', catalog: '产品目录', series: '系列', modelCount: '个型号', productModels: '产品型号', contactTitle: '告诉我们您的项目需求。',
...
function applySeo(route) {
  ...
  const title = route.series ? `${displayTitle(seriesTitle(route.series, route.locale))} | Synstro` : route.product ? `${route.product[route.locale].title} | Synstro` : isZh ? 'Synstro 网络机柜、PDU 与户外柜产品目录' : 'Synstro Network Cabinet, PDU, Outdoor Cabinet Catalog';
  const description = route.series ? `${displayTitle(seriesTitle(route.series, route.locale))} model selector, quote request, and full parameter table from Synstro.` : route.product ? route.product[route.locale].body : isZh ? 'Synstro 产品目录按分类、系列和型号展示网络机柜、PDU 与户外防水柜参数。' : 'Synstro catalog organized by category, series, model, and specifications for network cabinets, PDUs, and outdoor cabinets.';
...
function App() {
  ...
  const navItems = useMemo(() => [
    { label: t.nav.home, href: `/${route.locale}` }, { label: t.nav.products, href: `/${route.locale}/products` }, { label: t.nav.applications, href: `/${route.locale}/applications` }, { label: t.nav.advantages, href: `/${route.locale}/advantages` }, { label: t.nav.reviews, href: `/${route.locale}/reviews` }, { label: t.nav.about, href: `/${route.locale}/about` }, { label: t.nav.inquiry, href: `/${route.locale}/contact` }
  ], [route.locale, t]);
...
function ProductsOverview({ locale, nav, compact = false }) { const t = copy[locale]; return <section id="products" className={compact ? '' : 'catalog-page'}><p className="eyebrow">{t.catalog}</p><h2>{locale === 'en' ? 'Product directory under Products' : '“产品”下的目录结构'}</h2><div className="cards product-grid">{products.map((product) => <ProductCard key={product.slug} product={product} locale={locale} nav={nav} />)}</div></section>; }
function ProductCard({ product, locale, nav }) { const t = copy[locale]; const series = productSeries(product); return <article className="product-card"><ProductImage src={product.asset} alt={product[locale].title} /><small>{product[locale].eyebrow}</small><h3>{product[locale].title}</h3><p>{product[locale].body}</p><ul><li><b>{copy[locale].series}</b><span>{series.length}</span></li><li><b>{locale === 'en' ? 'Models' : '型号'}</b><span>{totalModels(product)}</span></li><li><b>{locale === 'en' ? 'Directory' : '目录'}</b><span>/products/{product.slug}</span></li></ul><button onClick={() => nav(productPath(locale, product))}>{t.view} →</button></article>; }
function ProductPage({ locale, product, nav }) { const t = copy[locale]; const series = productSeries(product); return <main><section className="page-hero product-family-hero"><div><p className="eyebrow">{product[locale].eyebrow}</p><h1>{product[locale].title}</h1><p className="lead">{product[locale].body}</p><button className="primary" onClick={() => nav('/' + locale + '/contact?product=' + product.slug)}>{t.cta}</button></div><ProductImage src={product.asset} alt={product[locale].title} /></section><section className="catalog-section"><div className="section-head"><p className="eyebrow">{t.series}</p><h2>{locale === 'en' ? 'Choose a series to view models' : '选择系列查看型号'}</h2></div><div className="catalog-grid">{series.map((item) => <button key={item.slug} className="series-card" onClick={() => nav(seriesPath(locale, item))}><span>{displayTitle(seriesTitle(item, locale))}</span><b>{item.models.length} {t.modelCount}</b><small>/products/{item.parent}/{item.slug}</small></button>)}</div></section><CTA locale={locale} nav={nav} product={product[locale].title} /></main>; }
