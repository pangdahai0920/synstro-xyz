(() => {
  ...
  function pathFor(category, item) {
    return `/${locale()}/products/${category.slug}/${item[0]}`;
  }
  ...
  function buildMenu(productsLink) {
    ...
    panel.innerHTML = `
      <div class="mega-primary" role="tablist"></div>
      <div class="mega-secondary" role="tabpanel"></div>
    `;
    ...
    function renderSecondary(index) {
      ...
      secondary.innerHTML = `
        <a class="mega-all" href="/${lang}/products/${category.slug}">${lang === 'zh' ? '查看全部' : 'View all'} ${category[lang]}</a>
        <div class="mega-list"></div>
      `;
    ...
    categories.forEach((category, index) => {
      ...
      button.addEventListener('click', () => {
        if (window.matchMedia('(max-width: 760px)').matches) {
          const wasActive = activeIndex === index && wrapper.classList.contains('open');
          renderSecondary(index);
          setOpen(!wasActive || !wrapper.classList.contains('open'));
        } else {
          location.href = `/${lang}/products/${category.slug}`;
        }
      });
  ...
  function enhance() {
    if (enhanced) return;
    const productsLink = document.querySelector('.plain-nav a[href$="/products"]');
